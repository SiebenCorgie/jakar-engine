...
impl Renderer {
    ...
    pub fn render(
        ...
        //now we can end the frame by switching internaly to the las pass and process the hdr image
        let finished_command_buffer = self.frame_system.preform_post_progress(command_buffer);
    ...
    where AutoCommandBufferBuilder: Sized + 'static
    {
        ...
        let command_buffer = {
            match frame_stage{
                frame_system::FrameStage::First(cb) => cb,
        ...
        let mesh_locked = match node.value{
            ...
            _ => return frame_system::FrameStage::First(command_buffer), //is no mesh :(
        };
        ...
        let new_cb = command_buffer
            ...
            frame_system::FrameStage::First(new_cb)
    }
